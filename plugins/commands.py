...
buttons = [
                    [
                        InlineKeyboardButton('⁉️ Support Group ⁉️', url='https://t.me/joinchat/vii7DDEvKCZkNDVl'),
                    ],
                    [
...
async def start(client, message):
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        ...
        if message.command[1] != "subscribe":
            btn.append([InlineKeyboardButton(" 🔄 Try Again after 🔴🔴Join Updates Channel🔴🔴  🔄", callback_data=f"checksub#{message.command[1]}")])
        await client.send_message(
            chat_id=message.from_user.id,
            text="**📣🤖🔴 𝗝𝗼𝗶𝗻 𝗨𝗽𝗱𝗮𝘁𝗲𝘀 𝗖𝗵𝗮𝗻𝗻𝗲𝗹 🤖🔴               𝗪𝗶𝘁𝗵𝗼𝘂𝘁 𝗝𝗢𝗜𝗡 𝘁𝗵𝗶𝘀 𝗰𝗵𝗮𝗻𝗻𝗲𝗹 𝘆𝗼𝘂 𝗰𝗮𝗻𝗻𝗼𝘁 𝘂𝘀𝗲 𝗯𝗼𝘁 🤖🔴 **",
