...
async def next_page(bot, query):
    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("⏪ 𝗕𝗔𝗖𝗞", callback_data=f"next_{req}_{key}_{off_set}"), InlineKeyboardButton(f"📃 Pages {round(int(offset)/10)+1} / {round(total/10)}", callback_data="pages")]
        )
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons= [[
                    InlineKeyboardButton('🕵 Creator 🕵', url='https://t.me/joinchat/vii7DDEvKCZkNDVl')
...
𝗦𝗼𝗿𝗿𝘆 𝗕𝗿𝗼 ,𝗡𝐎 𝐌𝐎𝐕𝐈𝐄𝐒/𝗦𝗲𝗿𝗶𝗲𝘀 𝗥𝗲𝗹𝗮𝘁𝗲𝗱 𝘁𝗼 𝘁𝗵𝗲 𝗚𝗶𝘃𝗲𝗻 𝗪𝗼𝗿𝗱 𝗪𝗮𝘀 𝗙𝗼𝘂𝗻𝗱                                                                                                                                     🥺
     <i>🥺  \n Join This Bot First for Download 👉 @PowerFilterRoBot \n\n 𝗣𝗹𝗲𝗮𝘀𝗲 𝐖𝐫𝐢𝐭𝐞 🔥 𝐌𝐎𝐕𝐈𝐄𝐒 🔥 𝐍𝐚𝐦𝐞 𝐖𝐢𝐭𝐡 𝐂𝐨𝐫𝐫𝐞𝐜𝐭 𝐒𝐩𝐞𝐥𝐥𝐢𝐧𝐠 𝐨𝐫 𝐘𝐨𝐮 𝐦𝐚𝐲 𝐧𝐨𝐭 𝐠𝐞𝐭 𝐲𝐨𝐮𝐫 𝐑𝐞𝐪𝐮𝐞𝐬𝐭   \n\n 𝐨𝐫 𝐏𝐥𝐞𝐚𝐬𝐞 𝐖𝐚𝐢𝐭...  \n📤📤 𝐅𝐢𝐥𝐞 𝐢𝐬 𝐔𝐩𝐥𝐨𝐚𝐝𝐢𝐧𝐠?📤📤     \n 📣 @PowerFilterRoBot  📣  </i></b>""",
        ...
        if offset != "":
            ...
            btn.append(
                [InlineKeyboardButton(text=f"🗓 1/{round(int(total_results)/10)}",callback_data="pages"), InlineKeyboardButton(text="𝗡𝗘𝗫𝗧 ⏩",callback_data=f"next_{req}_{key}_{offset}")]
        ...
        if imdb and imdb.get('poster'):
            ...
            await message.reply_text(f"<b> \n ⚡Join This Bot First for Download👉 @PowerFilterRoBot \n🔍 𝗛𝗲𝗿𝗲 𝗶𝘀 𝗥𝗲𝘀𝘂𝗹𝘁𝘀 𝗙𝗼𝗿 𝗬𝗼𝘂𝗿 𝗥𝗲𝗾𝘂𝗲𝘀𝘁  📣  @PowerFilterRoBot  📣  {search} ‌‌‌‌‎ 🔍</b>", reply_markup=InlineKeyboardMarkup(btn))
